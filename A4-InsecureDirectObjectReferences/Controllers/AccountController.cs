...
namespace A4_InsecureDirectObjectReferences.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult GetMessage(string userId, int messageId)
        {
            ...
            using (SqlCommand command = new SqlCommand("select * from Messages where UserId = @User AND Id = @Message", connection))
            {
                command.Parameters.Add(new SqlParameter { ParameterName = "User", Value = userId });
                command.Parameters.Add(new SqlParameter { ParameterName = "Message", Value = messageId });
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        return this.Json(new Message
                        {
                            ...
                            Content = reader["Message"].ToString().Trim(),
